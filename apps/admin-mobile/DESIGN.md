---
version: alpha
name: Fixer-brand-system
description: Fixer home-services brand — deep navy primary (#00142F), amber secondary (#FE9D16), cool neutrals on white canvas, pill-shaped interactive elements (radius 999px), and editorial service imagery. Navy carries trust and CTAs; amber highlights promos, badges, and accent actions.
...
### Do
- Reserve `{colors.primary}` (`#00142F`) for primary CTA pills, selected chips, and key navigation active states. One navy pill per viewport anchors conversion.
- Use `{colors.secondary}` (`#FE9D16`) sparingly for accent CTAs (`button-brand`), promo badges, star ratings, and highlights — not full-page fills.
- Pair amber buttons with `{colors.on-secondary}` (`#00142F`) text for readable contrast; navy buttons use `{colors.on-primary}` white text.
- Use `{rounded.pill}` 999 px on every interactive element (buttons, chips, app pills). The pill IS the brand's geometric signature.
- Render cards in `{rounded.xl}` 16 px — promo cards, content cards, and form cards share this radius.
- Set every headline in `{typography.display-*}` weight 700 in sentence-case. Body copy stays in `{colors.body}` gray, not navy.
- Use navy-tinted hero overlays (`rgba(0, 20, 47, *)`) on imagery; reserve dark bands for `{colors.ink}` / `{colors.primary}`.

### Don't
- Don't flood screens with amber — secondary is an accent, not a background wash.
- Don't use white text on amber for small labels (contrast fails); use navy on amber or white on navy.
- Don't render the primary CTA as a `{rounded.xl}` rectangle except inside documented large-form flows.
- Don't use all-caps display headlines. Sentence-case is the voice; uppercase is restricted to rare eyebrow tags.
- Don't drop a soft drop-shadow on every card. Level 0 flat is the default; shadow is reserved for floating pills and elevated forms.
- Don't introduce third accent colours (green, pink, etc.) outside semantic success/error/warning tokens.
- Don't use `{rounded.full}` 9999 px for square cards — cards stay at `{rounded.xl}` 16 px.

---

## Implementation map (DESIGN.md → TypeScript)

The brand system above is the **single source of truth** for the mobile app.
Every value is implemented in TypeScript under `apps/admin-mobile/src/theme/`
and consumed via UI primitives. **Do not introduce raw hex / px values in
component files** — extend `theme/tokens.ts` instead.

### Token files

| Token group       | File                                | Exports                           |
| ----------------- | ----------------------------------- | --------------------------------- |
| Colors / palette  | `src/theme/tokens.ts`               | `palette`                         |
| Spacing scale     | `src/theme/tokens.ts`               | `spacing`                         |
| Border radius     | `src/theme/tokens.ts`               | `radius`                          |
| Typography ramp   | `src/theme/tokens.ts`               | `typography`                      |
| Elevation levels  | `src/theme/tokens.ts`               | `elevation`                       |
| Hit-target sizes  | `src/theme/tokens.ts`               | `hitTarget`                       |
| Public re-export  | `src/theme/index.ts`                | all of the above                  |
| Legacy shim       | `src/theme/colors.ts` *(deprecated)*| maps old `colors.*` → `palette.*` |

### Primitive components (DESIGN.md component → file)

| DESIGN.md component                                                | File                                              |
| ------------------------------------------------------------------ | ------------------------------------------------- |
| `button-primary` / `secondary` / `subtle` / `brand` / `ghost`      | `src/components/ui/Button.tsx`                    |
| `text-input`, `text-input-on-soft`, password-toggle field          | `src/components/ui/Input.tsx` (`Input`, `PasswordInput`) |
| `card-content`, `card-soft-tinted`, `card-elevated`, `promo-card-on-dark` | `src/components/ui/Card.tsx`               |
| Typography ramp (`display-*`, `body-*`, `caption`)                 | `src/components/ui/Text.tsx`                      |
| Feather-style stroke icons (24-px viewBox, brand colour aware)     | `src/components/ui/Icon.tsx`                      |
| `nav-bar` page chrome / safe-area / canvas/soft/ink surface        | `src/components/layout/Screen.tsx`                |
| Status pill (semantic tone mapping)                                | `src/components/common/StatusBadge.tsx`           |
| KPI tile with `icon-button-circular` bubble                        | `src/components/common/KpiCard.tsx`               |
| Row card with chevron + meta                                       | `src/components/common/ListRow.tsx`               |
| Loading / error / empty container                                  | `src/components/common/QueryState.tsx`            |
| Empty-state illustration frame                                     | `src/components/common/EmptyState.tsx`            |
| Section header (display-md / display-lg)                           | `src/components/common/ScreenHeader.tsx`          |

### Rules of engagement

- **Always** read colours, spacing, radii, type from `@/theme`. Never hard-code.
- **Always** wrap text in `<Text variant="…" color="…" />` from `@/components/ui/Text` —
  it enforces the type ramp and palette in one place.
- **Always** wrap icons in `<Icon name="…" />`. Add new glyphs to `Icon.tsx`
  (path data from feathericons.com, 24-px viewBox, stroke-linecap round).
- New screens use `<Screen surface="canvas">` (or `"soft"`) — never set
  background colours by hand.
- New buttons use `<Button variant="primary|secondary|subtle|brand|ghost">` —
  the variant decides shape, padding, fg/bg, and elevation per DESIGN.md.
- Cards use `<Card variant="content|soft|elevated|onDark">`. The default
  state is `Level 0 — Flat` per DESIGN.md §Elevation.
- When a screen needs a token that doesn't exist, **add it to `tokens.ts`
  and document it here** — don't reach for a one-off hex value.

### Rebuild guidance

The mobile app uses `react-native-svg` (icons) and `react-native-config`
(env). After updating `package.json` deps you must run:

```bash
cd apps/admin-mobile
npm install
cd android && ./gradlew clean && cd ..   # only if a previous build cached stale autolinking
npm run android                           # or: npm run ios && cd ios && pod install
```

Pure JS / theme changes only require a Metro reload (`r` in the Metro
terminal).
