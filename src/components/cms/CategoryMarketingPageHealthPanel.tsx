...
const GROUP_ORDER = ['Metadata', 'Hero', 'Content', 'Local', 'Links & FAQs', 'Technical'] as const
...
export function CategoryMarketingPageHealthPanel({ report, lengthWarnings = [], onNavigateTab, className }: Props) {
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="space-y-1">
            ...
            <CardDescription className="text-xs">
              Content completeness + length checks for this CMS key.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4 pt-0">
        <div>
          ...
          <p className="mt-2 text-[11px] text-muted-foreground leading-snug">
            ~<strong className="font-medium text-foreground">{report.wordCountEstimate.toLocaleString()}</strong> words
            across hero, cards, guide &amp; FAQs
          </p>
        </div>
        ...
        {missingRequired > 0 ? (
          <div className="flex gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-foreground">
            ...
            <span>
              ...
              {missingRequired === 1 ? 'item' : 'items'} open before this page is publish-ready.
            </span>
          </div>
        ) : (
